"""Posts a single boot-state transition to the OpenSchool server.

Called by the systemd oneshot units and the PAM session hook in the image.
Designed to be invoked as `python3 -m openschool.reporter.report <state> [detail]`.
"""
...
def main(argv: list[str]) -> int:
    ...
    if not argv or len(argv) > 2:
        print(
            "usage: python3 -m openschool.reporter.report <state> [detail]",
            file=sys.stderr,
        )
        return 2
    try:
